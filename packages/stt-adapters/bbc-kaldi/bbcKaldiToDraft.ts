 ...
 */
const groupWordsInParagraphs = (words: Word[]): Paragraph[] => {
  const results: Paragraph[] = [];
  let paragraph: Paragraph = { words: [], text: [] };
 ...
 */
const bbcKaldiToDraft = (bbcKaldiJson: BbcKaldiJson): DraftJsContentBlock[] => {
  const results: DraftJsContentBlock[] = [];
  let tmpWords: Word[];
  let speakerSegmentation: SpeakerSegment[] | null = null;
  let wordsByParagraphs: Paragraph[] = [];
