...
const MediaPlayer: React.FC<MediaPlayerProps> = ({
  ...
  useEffect(() => {
    if (hookSeek) hookSeek(setCurrentTime);
    if (hookPlayMedia) hookPlayMedia(togglePlayMedia);
    if (hookIsPlaying) hookIsPlaying(isPlaying);
  ...
  const setTimeCodeOffset = useCallback((newTimeCodeOffSet: number) => {
  ...
  const setPlayBackRate = useCallback((input: number) => {
    const video = videoRef.current;
    if (video) {
  ...
  const handleChangeReplayRollbackValue = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const playerControls = useMemo(() => (
    <PlayerControls
      ...
      playbackRateOptions={state.playbackRateOptions}
      isMute={state.isMute}
      onPlayPause={togglePlayMedia}
      onSkipForward={skipForward}
      onSkipBackward={skipBackward}
      onPlaybackRateChange={setPlayBackRate}
      onMuteToggle={handleMuteVolume}
      onPictureInPicture={handlePictureInPicture}
      onProgressBarClick={handleProgressBarClick}
      onRollBack={rollBack}
      onSetCurrentTime={promptSetCurrentTime}
  ...
  const progressBar = useMemo(() => (
    <ProgressBar
      currentTime={getProgressBarValue()}
      duration={getProgressBarMax()}
      onProgressBarClick={handleProgressBarClick}
    />
  ), [getProgressBarValue, getProgressBarMax, handleProgressBarClick]);
