...
const draftToWhisper = blockData => {
    ...
    return segments
        ...
        const speakerLabel = speakerValue === undefined || speakerValue === null
            ? ''
            : speakerValue.toString().trim();
        const resolvedSpeaker = speakerLabel.length > 0 ? speakerLabel : `TBC ${index}`;
        ...
        const start = (normalisedWords[0] && asNumber(normalisedWords[0].start)) || 0;
        const end = (normalisedWords[normalisedWords.length - 1] &&
